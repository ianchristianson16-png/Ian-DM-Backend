...
// ── ManyChat bulk sync ────────────────────────────────────────────
app.post('/manychat/sync', async (req, res) => {
  const apiKey = process.env.MANYCHAT_API_KEY;
  if (!apiKey) return res.status(500).json({ error: 'MANYCHAT_API_KEY not set' });
  let imported=0, skipped=0, errors=0, page=1;
  try {
    while (true) {
      const mcRes = await fetch(
        `https://api.manychat.com/fb/subscriber/getSubscribers?page=${page}&count=100`,
        { headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' } }
      );
      if (!mcRes.ok) { console.error('ManyChat API error:', mcRes.status, await mcRes.text()); break; }
      const mcData = await mcRes.json();
      const subscribers = mcData.data || [];
      if (!subscribers.length) break;
      for (const sub of subscribers) {
        try {
          const igUsername = sub.instagram?.username || sub.instagram_username;
          const leadHandle = igUsername ? '@'+igUsername : '@'+(sub.name||sub.id).toLowerCase().replace(/\s+/g,'_');
          const existing = await pool.query(`SELECT id FROM conversations WHERE lead_handle=$1 LIMIT 1`,[leadHandle]);
          if (existing.rows.length > 0) { skipped++; continue; }
          const status = sub.status==='unsubscribed' ? 'ghosted' : 'active';
          const createdAt = sub.subscribed_at ? new Date(sub.subscribed_at*1000).toISOString() : new Date().toISOString();
          const result = await pool.query(
            `INSERT INTO conversations (lead_handle,platform,stage,status,notes,created_at,updated_at) VALUES ($1,'instagram','opened',$2,$3,$4,$4) RETURNING id`,
            [leadHandle, status, `Imported from ManyChat. Name: ${sub.name||'unknown'}. Status: ${sub.status||'unknown'}.`, createdAt]
          );
          await pool.query(
            `INSERT INTO messages (conversation_id,role,content,stage_at_time) VALUES ($1,'system',$2,'opened')`,
            [result.rows[0].id, `Imported from ManyChat bulk sync. ManyChat ID: ${sub.id}`]
          );
          imported++;
        } catch(rowErr) { console.error('Row error:',rowErr.message); errors++; }
      }
      console.log(`Sync page ${page}: ${imported} imported so far`);
      if (subscribers.length < 100) break;
      page++;
      await new Promise(r=>setTimeout(r,300));
    }
    setTimeout(autoWriteKPI,1000);
    res.json({ success:true, imported, skipped, errors, total:imported+skipped+errors, message:`Sync complete. ${imported} new contacts imported, ${skipped} already existed.` });
  } catch(err) { console.error('Sync error:',err.message); res.status(500).json({error:err.message,imported,skipped}); }
});

app.get('/manychat/sync/status', async (req, res) => {
  try {
    const r = await pool.query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='active') as active, COUNT(*) FILTER (WHERE status='ghosted') as ghosted, COUNT(*) FILTER (WHERE is_icp=true) as icp, COUNT(*) FILTER (WHERE notes LIKE '%ManyChat bulk sync%') as from_manychat FROM conversations`);
    res.json(r.rows[0]);
  } catch(err) { res.status(500).json({error:err.message}); }
});
