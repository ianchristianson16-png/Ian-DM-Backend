...
async function generateInsights() {
  ...
  try {
    const convData = await pool.query(`
      SELECT c.id, c.status, c.stage, c.situation, c.created_at, c.call_booked_at,
    ...
    const summary = { total: convData.rows.length, booked: convData.rows.filter(r=>r.status==='booked').length, ghosted: convData.rows.filter(r=>r.status==='ghosted').length };
    ...
    const prompt = `You are analyzing DM conversation data for Ian, a 19-year-old entrepreneur coaching people on communication and confidence. He books strategy calls through Instagram/TikTok DMs.
Last 30 days: ${summary.total} conversations, ${summary.booked} booked, ${summary.ghosted} ghosted.
...
async function sendWeeklyReport() {
  ...
  try {
    const stats = await pool.query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='booked') as booked, COUNT(*) FILTER (WHERE status='ghosted') as ghosted, COUNT(*) FILTER (WHERE stage IN ('pitch','schedule','confirm','booked')) as reached_pitch, ROUND(AVG(EXTRACT(EPOCH FROM (call_booked_at-created_at))/3600) FILTER (WHERE call_booked_at IS NOT NULL),1) as avg_hours FROM conversations WHERE created_at > NOW() - INTERVAL '7 days'`);
...
async function runFollowupCron() {
  try {
    ...
    for (const conv of result.rows) {
      ...
      const fc = parseInt(conv.followup_count)||0;
      const lt = conv.last_followup_type;
      let ft = null;
...
function scheduleWeeklyReport() {
  const now = new Date();
  const pst = new Date(now.toLocaleString('en-US',{timeZone:IAN_TZ}));
...
app.get('/kpi', async (req,res) => {
  try {
    ...
    result.rows.forEach(row => {
      if (W[row.view_type]) W[row.view_type].push({ label: row.period_label, ...row.data });
    });
    const hasData = Object.values(W).some(arr => arr.length > 0);
    res.json({ data: W, hasData });
...
app.post('/kpi', async (req,res) => {
  ...
  try {
    await pool.query(`
      ...
      VALUES ('ian', $1, $2, $3, $4, NOW())
      ON CONFLICT (user_id, view_type, period_label)
      DO UPDATE SET data=$4, period_index=$3, updated_at=NOW()
    `, [viewType, periodLabel, periodIndex, JSON.stringify(data)]);
      ...
      for (const viewType of ['daily','weekly','monthly','yearly']) {
        ...
        for (let i=0; i<periods.length; i++) {
          ...
          await client.query(`
            ...
            ON CONFLICT (user_id, view_type, period_label)
            DO UPDATE SET data=$4, period_index=$3, updated_at=NOW()
          `, [viewType, label, i, JSON.stringify(data)]);
        }
      }
...
app.post('/conversation/message', async (req,res) => {
  ...
  try { await pool.query(`INSERT INTO messages (conversation_id,role,content,stage_at_time) VALUES ($1,$2,$3,$4)`,[conversationId,role,content,stage]); res.json({success:true}); }
  catch(err){res.status(500).json({error:err.message});}
});
...
app.get('/analytics/stats', async (req,res) => {
  try {
    ...
    const weekly=await pool.query(`SELECT DATE_TRUNC('week',created_at) as week, COUNT(*) as started, COUNT(*) FILTER (WHERE status='booked') as booked, COUNT(*) FILTER (WHERE status='ghosted') as ghosted FROM conversations GROUP BY week ORDER BY week DESC LIMIT 8`);
    const insightRow=await pool.query(`SELECT insights FROM insights_cache ORDER BY generated_at DESC LIMIT 1`);
...
app.get('/conversations', async (req,res) => {
  try { const r=await pool.query(`SELECT c.*,COUNT(m.id) as message_count,COUNT(f.id) FILTER (WHERE f.status='queued') as pending_followups FROM conversations c LEFT JOIN messages m ON m.conversation_id=c.id LEFT JOIN followups f ON f.conversation_id=c.id GROUP BY c.id ORDER BY c.updated_at DESC LIMIT 100`); res.json({conversations:r.rows}); }
  catch(err){res.status(500).json({error:err.message});}
});
