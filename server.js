  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS weekly_stats (
        ...
        reached_pitch INTEGER DEFAULT 0,
        calls_booked INTEGER DEFAULT 0,
        ghosted INTEGER DEFAULT 0,
        declined INTEGER DEFAULT 0,
        avg_time_to_book_hours NUMERIC,
        created_at TIMESTAMPTZ DEFAULT NOW()
      );
    `);
...
// Mark as ghosted / declined
app.post('/conversation/status', async (req, res) => {
  const { conversationId, status } = req.body;
  try {
    const timestampField = status === 'ghosted' ? 'ghosted_at' : status === 'declined' ? 'declined_at' : null;
    if (timestampField) {
      await pool.query(
        `UPDATE conversations SET status = $1, ${timestampField} = NOW(), updated_at = NOW() WHERE id = $2`,
...
app.post('/book', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error('DB booking log error:', dbErr.message);
    }
    ...
    try {
      ...
      const body = `New strategy call booked via IAN.DM\n\nLead: ${leadName}\nTime: ${startFormatted}\nSituation: ${situation}\n\nCalendar: ${event.data.htmlLink}`;
...
app.get('/analytics/stats', async (req, res) => {
  try {
    ...
    const recentBookings = await pool.query(`
      SELECT lead_handle, call_datetime, situation, created_at
      FROM bookings
...
app.get('/conversations/:id', async (req, res) => {
  try {
    ...
    res.json({ conversation: conv.rows[0], messages: messages.rows, transitions: transitions.rows });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
